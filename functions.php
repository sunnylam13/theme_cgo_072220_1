...
add_action( 'wp_enqueue_scripts', 'cgo_scripts' );


/* Custom Title Tags */

function cgo_wp_title( $title, $sep ) {
	global $paged, $page;
	...
	if ( ( $paged >= 2 || $page >= 2 ) && ! is_404() ) {
		$title = "$title $sep " . sprintf( __( 'Page %s', 'cgo' ), max( $paged, $page ) );
	}

	return $title;
}
add_filter( 'wp_title', 'cgo_wp_title', 10, 2 );

/*
  Get our wp_nav_menu() fallback, wp_page_menu(), to show a home link.
 */
function cgo_page_menu_args( $args ) {
	$args['show_home'] = true;
	return $args;
}
add_filter( 'wp_page_menu_args', 'cgo_page_menu_args' );


/*
 * Sets the post excerpt length to 40 characters.
 */
function cgo_excerpt_length( $length ) {
	return 40;
}
add_filter( 'excerpt_length', 'cgo_excerpt_length' );

/*
 * Returns a "Continue Reading" link for excerpts
 */
function cgo_continue_reading_link() {
	return ' <a href="'. get_permalink() . '">Continue reading <span class="meta-nav">&rarr;</span></a>';
}

/**
 * Replaces "[...]" (appended to automatically generated excerpts) with an ellipsis and cgo_continue_reading_link().
 */
function cgo_auto_excerpt_more( $more ) {
	return ' &hellip;' . cgo_continue_reading_link();
}
add_filter( 'excerpt_more', 'cgo_auto_excerpt_more' );

/**
 * Adds a pretty "Continue Reading" link to custom post excerpts.
 */
function cgo_custom_excerpt_more( $output ) {
	if ( has_excerpt() && ! is_attachment() ) {
		$output .= cgo_continue_reading_link();
	}
	return $output;
}
add_filter( 'get_the_excerpt', 'cgo_custom_excerpt_more' );


/*
 * Register a single widget area.
 * You can register additional widget areas by using register_sidebar again
 * within cgo_widgets_init.
 * Display in your template with dynamic_sidebar()
 */
function cgo_widgets_init() {
...
add_action( 'widgets_init', 'cgo_widgets_init' );

/**
 * Removes the default styles that are packaged with the Recent Comments widget.
 */
function cgo_remove_recent_comments_style() {
	global $wp_widget_factory;
	remove_action( 'wp_head', array( $wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style' ) );
}
add_action( 'widgets_init', 'cgo_remove_recent_comments_style' );


if ( ! function_exists( 'cgo_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post—date/time and author.
 */
function cgo_posted_on() {
...
if ( ! function_exists( 'cgo_posted_in' ) ) :
/**
 * Prints HTML with meta information for the current post (category, tags and permalink).
 */
function cgo_posted_in() {
